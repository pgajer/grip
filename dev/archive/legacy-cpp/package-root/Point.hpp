...
template<class T>
unsigned long Point<T>::norm() const {
    return ROUND_L(std::sqrt(static_cast<double>(norm2())));
}

template<class T>
float Point<T>::fnorm() const {
    return std::sqrt(static_cast<float>(norm2()));
}

template<class T>
unsigned long Point<T>::norm2() const {
    unsigned long result = 0;
    for(size_t ind = 0; ind < dim; ind++){
        result += static_cast<unsigned long>(coord[ind]) * static_cast<unsigned long>(coord[ind]);
    }
    return result;
}

template<class T>
float Point<T>::fnorm2() const {
    float result = 0;
    for(size_t ind = 0; ind < dim; ind++)
        result += static_cast<float>(coord[ind]) * static_cast<float>(coord[ind]);
    return result;
}
