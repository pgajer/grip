...
This distinction matters for the paper. grip now includes pure geodesic-KK
tools, standalone landmark geodesic-KK tools, a standalone MISF-based
geodesic-KK family, and MISF-based GMDS tools. The new MISF-GKK family extends
the package's multiscale geodesic toolkit, especially for weighted graphs where
coarse-to-fine geodesic optimization is useful. Even so, this package paper
keeps the main focus on the `grip.layout*()` interfaces, because they remain
the clearest user-facing expression of the package's main strength: fast 2D and
3D graph layout at practical runtime scale.

For advanced users, `grip.prepare/score/optimize.misf.geodesic.kk()` now
provides a multistage pipeline that combines MISF coarsening, top-level
geodesic-KK or landmark geodesic-KK solving, coarse-to-fine insertion, anchored
active-level refinement, and final geodesic polish. That family is important
for the package's longer-term weighted-layout scope, but it is secondary to the
main manuscript story, which is routine large-graph layout and assessment in R.
...
The package now also includes a fuller advanced geodesic toolbox than the main
text emphasizes, including exact GKK, sparse LGKK, MISF-GKK, and MISF-GMDS
families. That broader toolkit matters for users working with weighted graph
metrics, but for an R Journal package paper the most persuasive message is
still the practical one: `grip.layout*()` makes fast multiscale 2D and 3D
layout available in ordinary R workflows, and the scoring utilities make the
resulting choices inspectable.
