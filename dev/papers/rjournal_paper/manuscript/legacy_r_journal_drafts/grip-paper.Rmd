...
\code{r = 0.10}, `s = 4.5`, and `repulsion_factor = 1.5`. The tree preset
...
```{r karate-final, fig.cap="Zachary karate club with a tuned tree-derived 3D layout, colored by faction membership (green = Mr.\\ Hi, orange = Officer). Two orthographic views of the static layout.", fig.width=9, fig.height=4}
...
goals. The tuned **grip** configuration improves on the package defaults for
...
**grip** is aimed at larger graphs, structured graph families, and 3D layout
selection workflows. Its key advantage here is that `grip.score.layout()`
provides a common scoring framework that can assess layouts from any source,
making **grip** complementary to existing tools rather than a replacement for
them.
