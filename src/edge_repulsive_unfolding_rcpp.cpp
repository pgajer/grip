...
List grip_optimize_edge_repulsive_stage_cpp(NumericMatrix coords,
  ...
  std::vector<int> accepted, wall;

  auto push_trace = [&](int iter, const EdgeRepulsiveState& s, double step, bool accept) {
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    push_trace(iter, state, NA_REAL, true);
    ...
    push_trace(iter + 1, did_accept ? candidate_state : state, did_accept ? step : NA_REAL, did_accept);
    if (!did_accept) break;
    Z = candidate;
