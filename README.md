...
- Multiscale trace diagnostics for both workflows (`trace.grip()`,
  `trace.weighted.grip()`).
...
- Use `compare.layouts()` and `score.layout()` when the graph is
  ...
  run.
- Use `trace.grip()` or `trace.weighted.grip()` when you need to
...
`trace.grip()`. Starting from a coarse global placement, the algorithm
iteratively refines vertex positions until the layout converges.
