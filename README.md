...
multiscale graph layout in 2D and 3D, with dimension-general weighted
layouts for `dim >= 2`. The main workflow is:

- `grip.layout()` for unweighted graphs,
- `grip.layout.weighted()` for weighted graphs via the ND backend,
...
- Add GKK/LGKK only after you already have weighted candidate layouts
  ...
  experimental tools rather than the default starting point. The
  original 2D/3D weighted implementation remains available as
