...
output_html <- file.path(
  interactive_dir,
  "gmds_v2_paraboloid_regularized_strong_regularizers_all_embeddings_2026-04-01.html"
)
...
build_title_lines <- function(row, method_id) {
  if (identical(method_id, "anchor_repulsion")) {
    c(
      sprintf("iter %d", row$iteration[[1L]]),
      sprintf("lambda_A %s, lambda_R %s", fmt_num(row$lambda_A[[1L]], 2L), fmt_num(row$lambda_R[[1L]], 2L)),
      sprintf("sigma %s, rho %s", fmt_num(row$gmds_stress[[1L]], 4L), fmt_num(row$procrustes_rmse[[1L]], 4L)),
      sprintf("eta %s, alpha_0.05 %s", fmt_num(row$roughness[[1L]], 4L), fmt_num(row$area_q05_ratio[[1L]], 4L))
    )
  } else {
    c(
      sprintf("iter %d", row$iteration[[1L]]),
      sprintf("lambda_A %s, lambda_E %s", fmt_num(row$lambda_A[[1L]], 2L), fmt_num(row$lambda_E[[1L]], 2L)),
      sprintf("sigma %s, rho %s", fmt_num(row$gmds_stress[[1L]], 4L), fmt_num(row$procrustes_rmse[[1L]], 4L)),
      sprintf("eta %s, alpha_0.05 %s", fmt_num(row$roughness[[1L]], 4L), fmt_num(row$area_q05_ratio[[1L]], 4L))
    )
  }
...
method_labels <- c(
  anchor_repulsion = "Anchor-plus-repulsion regularized GMDS",
  anchor_edge_spring = "Anchor-plus-edge-spring regularized GMDS"
)
...
case_sections <- lapply(names(result_groups), function(key) {
  ...
  cards <- lapply(seq_len(nrow(traj)), function(i) {
    ...
    note <- if (identical(result$method_id, "anchor_repulsion")) {
      "Strong coupled schedule with anchor and repulsion both decaying linearly from 1.0 to 0.1 over 15 iterations."
    } else {
      "Strong coupled schedule with anchor and edge-spring weights both decaying linearly from 1.0 to 0.1 over 15 iterations."
    }
    card_div(build_title_lines(row, result$method_id), widget, note)
  })

  htmltools::tags$section(
    class = "case-section",
    htmltools::tags$h2(sprintf("%s: %s", case_info$label, method_labels[[result$method_id]])),
    htmltools::tags$p(
      class = "case-lead",
      "Every stored iterate is shown below after Procrustes alignment to the reference paraboloid, so the trajectory can be inspected as a shape evolution rather than a sequence of arbitrary rigid motions."
    ),
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: Strong Coupled Regularized-GMDS Trajectories"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
        class = "hero",
        htmltools::tags$h1("Interactive 3D Panels: Strong Coupled Regularized-GMDS Trajectories"),
        htmltools::tags$p(
          "This page shows every stored embedding from the two stronger coupled schedule experiments appended to the dedicated paraboloid regularized-GMDS report. The anchor-plus-repulsion and anchor-plus-edge-spring trajectories are both run with linear schedules from 1.0 to 0.1 over 15 iterations, and every layout is aligned to the reference surface before rendering."
        )
      ),
      case_sections
    )
