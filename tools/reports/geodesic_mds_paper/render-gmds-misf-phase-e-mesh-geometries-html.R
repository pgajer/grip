...
tmp_dir <- file.path(design_root, "tmp", "gmds-misf-phase-e-mesh-geometries-2026-04-02")
interactive_dir <- file.path(design_root, "interactive-prototypes")
output_html <- file.path(interactive_dir, "gmds_misf_phase_e_mesh_geometries_3d_2026-04-02.html")
...
case_sections <- lapply(bundle$case_results, function(case_result) {
  ...
  htmltools::tags$section(
    ...
    htmltools::tags$p(
      class = "case-lead",
      "All non-reference layouts are Procrustes-aligned to the case-specific reference geometry before rendering. The sections below show the corrected top-level seeds, the one-level weighted-KK lookahead used for candidate ranking, the final integrated pipelines, and stagewise views for the selected pipelines."
    ),
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: Phase E MISF-GMDS Mesh Geometries"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
        class = "hero",
        htmltools::tags$h1("Block 1: Phase E MISF-GMDS Across Mesh Geometries"),
        htmltools::tags$p(
          "This gallery expands the Phase E seeded MISF-GMDS study from paraboloids to flat, saddle, paraboloid, and ripple geometries on regular and irregular rectangle meshes. For each case it shows the corrected top-level seed pool, the one-level weighted-KK lookahead, the final integrated pipelines, and the selected stagewise trajectories."
        )
      ),
      case_sections
    )
...
message("Wrote Block 1 interactive HTML: ", output_html)
