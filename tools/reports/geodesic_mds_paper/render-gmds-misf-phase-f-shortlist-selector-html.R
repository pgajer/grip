...
tmp_dir <- file.path(design_root, "tmp", "gmds-misf-phase-f-shortlist-selector-2026-04-05")
interactive_dir <- file.path(design_root, "html")
output_html <- file.path(interactive_dir, "gmds_misf_phase_f_shortlist_selector_3d_2026-04-05.html")
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: Phase F Shortlist Selector for Seeded MISF-GMDS"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
      class = "hero",
      htmltools::tags$h1("Phase F: Shortlist Selector Improvement for Seeded MISF-GMDS"),
      htmltools::tags$p(
        "This gallery keeps the seeded MISF-GMDS pipeline fixed and studies only the selector. For each regular or irregular rectangle mesh geometry case it shows the corrected top-level seed pool, the original Phase E one-level weighted-KK lookahead, the Phase F second-lookahead shortlist on the top two Phase E candidates, the final integrated pipelines, and the selected stagewise trajectories."
      )
    ),
      case_sections
    )
...
message("Wrote Phase F interactive HTML: ", output_html)
