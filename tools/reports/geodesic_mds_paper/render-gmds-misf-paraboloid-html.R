...
tmp_dir <- file.path(manual_root, "tmp", "gmds-misf-paraboloid-2026-04-02")
interactive_dir <- file.path(manual_root, "html")
output_html <- file.path(
  interactive_dir,
  "gmds_misf_paraboloid_3d_panels_2026-04-02.html"
)
...
method_note <- function(case_result, row) {
  ...
  if (grepl("^misf_", row$method_id[[1L]])) {
    stage_rows <- ifelse(is.finite(row$stage_rows[[1L]]), row$stage_rows[[1L]], 0L)
    return(sprintf("%s. n = %d vertices. MISF trace rows: %d.", row$method_label[[1L]], row$n[[1L]], stage_rows))
  }
...
case_sections <- lapply(bundle$case_results, function(case_result) {
  ...
  htmltools::tags$section(
    ...
    htmltools::tags$p(
      class = "case-lead",
      "All non-reference layouts are Procrustes-aligned to the reference paraboloid before rendering, so the panels differ by intrinsic shape rather than by rigid viewing transforms."
    ),
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: MISF-GMDS Paraboloid Validation"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
        class = "hero",
        htmltools::tags$h1("MISF-GMDS Paraboloid Validation: Interactive Panels"),
        htmltools::tags$p(
          "This page mirrors the Phase 6 paraboloid validation report. Each case shows the reference surface, the classical-MDS baseline, the cmdscale-initialized pure-GMDS baseline, the MISF multiscale run without final polish, and the MISF multiscale run with the final full-graph polish."
        ),
        htmltools::tags$p(
          "The intention is to inspect basin behavior directly in 3D. Regular and irregular paraboloids are both included, because the earlier experiments suggested that irregularity changes the optimizer's basin structure even when it does not eliminate the underlying fold-blindness of the path objective."
        )
      ),
      case_sections
    )
...
htmltools::save_html(
  page,
  file = output_html,
  libdir = paste0(output_html, "_files"),
  background = "white"
)
