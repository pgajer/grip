...
tmp_dir <- file.path(design_root, "tmp", "gmds-misf-phase-g-deferred-portfolio-2026-04-05")
interactive_dir <- file.path(design_root, "interactive-prototypes")
output_html <- file.path(interactive_dir, "gmds_misf_phase_g_deferred_portfolio_3d_2026-04-05.html")
...
case_sections <- lapply(bundle$case_results, function(case_result) {
  portfolio_cards <- lapply(case_result$portfolio_results, function(entry) {
    card_div(
      title_text = sprintf(
        "%s\nG-rank %s\nsigma %s, rough %s\nedge floor %s, alpha %s, rho %s",
        entry$method_label,
        entry$phase_g_rank_sum,
        ...
        fmt_num(entry$final_rho, 4L)
      ),
      widget = make_widget(entry$display, case_result$case$edges),
      note = "Deferred full-portfolio candidate"
    )
  })
  ...
  lookahead_cards <- c(
    ...
    lapply(case_result$candidates, function(candidate) {
      ...
      card_div(
        ...
        "Original Phase E one-level weighted-KK lookahead"
      )
    })
  )
  ...
  shortlist_cards <- c(
    list(card_div("Reference surface", make_widget(case_result$case$truth, case_result$case$edges, vertex_col = "#bc6c25"))),
    lapply(case_result$shortlist_results, function(entry) {
      second.metrics <- entry$second_lookahead$metrics[1L, , drop = FALSE]
      card_div(
        sprintf(
          "%s\nE-rank %s, F-rank %s\n2-step sigma %s, rough %s, edge floor %s",
          entry$method_label,
          entry$phase_e_proxy_rank,
          case_result$shortlist_metrics$phase_f_rank_sum[[match(entry$method_id, case_result$shortlist_metrics$method_id)]],
        ...
        make_widget(entry$second_lookahead$display, case_result$case$edges),
        "Phase F second weighted-KK lookahead"
      )
    })
  )
  ...
  htmltools::tags$section(
    ...
    htmltools::tags$p(
      class = "case-lead",
      "All non-reference layouts are Procrustes-aligned to the case-specific reference geometry before rendering. The sections below show the corrected top-level seeds, the original Phase E one-level weighted-KK lookahead, the Phase F shortlist second lookahead, the full deferred Phase G portfolio ranking, the final integrated pipelines, and stagewise views for the selected pipelines."
    ),
    htmltools::tags$h3("Corrected Top-Level Seeds"),
    htmltools::tags$div(class = "panel-grid", top_cards),
    htmltools::tags$h3("Phase E One-Level Weighted-KK Lookahead"),
    htmltools::tags$div(class = "panel-grid", lookahead_cards),
    htmltools::tags$h3("Phase F Shortlist Second Lookahead"),
    htmltools::tags$div(class = "panel-grid", shortlist_cards),
    htmltools::tags$h3("Phase G Deferred Portfolio Ranking"),
    htmltools::tags$div(class = "panel-grid", portfolio_cards),
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: Phase G Deferred Portfolio Selection for Seeded MISF-GMDS"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
      class = "hero",
      htmltools::tags$h1("Phase G: Deferred Portfolio Selection for Seeded MISF-GMDS"),
      htmltools::tags$p(
        "This gallery keeps the seeded MISF-GMDS pipeline fixed and studies only the selector. For each regular or irregular rectangle mesh geometry case it shows the corrected top-level seed pool, the original Phase E one-level weighted-KK lookahead, the Phase F shortlist second lookahead on the top two Phase E candidates, the deferred Phase G portfolio ranking over all final candidates, the final integrated comparison, and the selected stagewise trajectories."
      )
    ),
      case_sections
    )
...
message("Wrote Phase G interactive HTML: ", output_html)
