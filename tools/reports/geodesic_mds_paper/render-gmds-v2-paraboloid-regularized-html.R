...
output_html <- file.path(
  interactive_dir,
  "gmds_v2_paraboloid_regularized_3d_panels_2026-04-01.html"
)
...
method_note <- function(row) {
  sprintf(
    "%s. Family: %s. Anchor energy %s, edge-spring energy %s, repulsion energy %s.",
    row$method_label[[1L]],
    row$family[[1L]],
    fmt_num(row$anchor_energy[[1L]], 4L),
    fmt_num(row$edge_spring_energy[[1L]], 4L),
    fmt_num(row$repulsion_energy[[1L]], 4L)
  )
}
...
case_sections <- lapply(bundle$case_results, function(case_result) {
  ...
  htmltools::tags$section(
    ...
    htmltools::tags$p(
      class = "case-lead",
      "All non-reference layouts are Procrustes-aligned to the reference paraboloid before rendering, so the panels differ only by intrinsic shape rather than by rigid rotations or reflections."
    ),
...
page <- htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$head(
      htmltools::tags$title("Interactive 3D Panels: Paraboloid Regularized-GMDS Study"),
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$section(
        class = "hero",
        htmltools::tags$h1("Interactive 3D Panels: Paraboloid Regularized-GMDS Study"),
        htmltools::tags$p(
          "This page mirrors the dedicated Step 7 report. It isolates the regularized-GMDS family on orthogonal paraboloid meshes, keeps the pure-GMDS and classical-MDS baselines visible, and exposes the exact lambda schedules and wall-clock embedding times for each 3D layout."
        )
      ),
      case_sections
    )
