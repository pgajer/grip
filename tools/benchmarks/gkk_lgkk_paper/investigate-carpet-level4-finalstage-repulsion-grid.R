...
if (requireNamespace("devtools", quietly = TRUE)) {
  ...
  stop("Install 'devtools' or the 'grip' package to run the carpet final-stage grid study.")
}
...
config_df <- expand.grid(
  ...
  repulsion_factor = repulsion_grid,
  stringsAsFactors = FALSE
)
config_df$config_id <- sprintf("fr%02d_r%s",
                               config_df$final_rounds,
                               gsub("\\.", "p", format(config_df$repulsion_factor, nsmall = 2)))
config_df <- config_df[, c("config_id", "final_rounds", "repulsion_factor")]
...
run_one <- function(config_row, seed) {
  timed <- time_it({
    grip.layout(
      ...
      repulsion_factor = config_row$repulsion_factor[[1L]]
    )
  })
...
message("Running FR final-stage grid on level-4 carpet...")
raw_metrics <- do.call(
  rbind,
  lapply(seq_len(nrow(config_df)), function(i) {
    config_row <- config_df[i, , drop = FALSE]
    message(sprintf("  %s (%d/%d)", config_row$config_id[[1L]], i, nrow(config_df)))
...
if (kk_available) {
  ...
  kk_row <- data.frame(
    ...
    elapsed_sec = kk_timed$elapsed,
    stringsAsFactors = FALSE
  )
}

smoothness_distance <- function(row, ref_row) {
  sqrt(
    ((row$edge_length_cv_mean - ref_row$edge_length_cv_mean) / ref_row$edge_length_cv_mean)^2 +
      ((row$edge_axis_deviation_mean - ref_row$edge_axis_deviation_mean) / ref_row$edge_axis_deviation_mean)^2 +
      ((row$boundary_waviness_mean - ref_row$boundary_waviness_mean) / ref_row$boundary_waviness_mean)^2
  )
}

summary_df$distance_to_f16 <- vapply(seq_len(nrow(summary_df)),
                                     function(i) smoothness_distance(summary_df[i, , drop = FALSE], ref_f16),
                                     numeric(1L))
summary_df$distance_to_f32 <- vapply(seq_len(nrow(summary_df)),
                                     function(i) smoothness_distance(summary_df[i, , drop = FALSE], ref_f32),
                                     numeric(1L))

near_kk <- summary_df[summary_df$procrustes_rmse_mean <= rmse_target_threshold, , drop = FALSE]
best_near_kk_f16 <- if (nrow(near_kk) > 0L) near_kk[which.min(near_kk$distance_to_f16), , drop = FALSE] else summary_df[0, , drop = FALSE]
best_near_kk_f32 <- if (nrow(near_kk) > 0L) near_kk[which.min(near_kk$distance_to_f32), , drop = FALSE] else summary_df[0, , drop = FALSE]
...
raw_csv_path <- file.path(output_root, "carpet-level4-finalstage-repulsion-grid-raw.csv")
...
lines <- c(
  ...
  sprintf("- sampled_f0 equivalent (`f1 r=2.5`) mean RMSE: `%s`",
          format_num(ref_f1$procrustes_rmse_mean, 4L)),
