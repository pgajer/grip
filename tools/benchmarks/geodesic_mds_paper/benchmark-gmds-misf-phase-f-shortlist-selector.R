...
run_tag <- if (smoke) {
  sprintf("gmds-misf-phase-f-shortlist-selector-smoke-%s", format(Sys.Date(), "%Y-%m-%d"))
} else {
...
rds_path <- file.path(tmp_dir, "gmds_misf_phase_f_shortlist_selector_results.rds")
selection_csv <- file.path(tmp_dir, "gmds_misf_phase_f_shortlist_selector_selection_metrics.csv")
final_csv <- file.path(tmp_dir, "gmds_misf_phase_f_shortlist_selector_final_metrics.csv")
summary_csv <- file.path(tmp_dir, "gmds_misf_phase_f_shortlist_selector_summary_metrics.csv")
shortlist_csv <- file.path(tmp_dir, "gmds_misf_phase_f_shortlist_selector_shortlist_metrics.csv")
...
run_candidate <- function(case, corrected_method) {
  ...
  final.metrics <- compute_final_metrics(
    ...
    note = "Phase F benchmark full weighted-KK lower-level placement pipeline"
  )
...
save_candidate_grid <- function(case_result, stage = c("top", "lookahead"), output_path) {
  ...
  ttl <- if (identical(stage, "top")) "Phase F corrected top-level seeds" else "Phase E one-level weighted-KK lookahead"
...
case_results <- lapply(cases, function(case) {
  message("Running Phase F shortlist selector study for: ", case$label)
  ...
  selected.ids <- unique(c(phase_e.method.id, phase_f.method.id, fixed.method.id, oracle.rho.id))
  ...
  comparison_entries <- list(
    ...
    list(
      method_id = phase_e.method.id,
      method_label = paste0("Phase E proxy winner (", corrected[[phase_e.method.id]]$method_label, ")"),
      display_coords = candidates[[phase_e.method.id]]$final_display,
      gmds_stress = candidates[[phase_e.method.id]]$final_metrics$gmds_stress[[1L]],
      procrustes_rmse = candidates[[phase_e.method.id]]$final_metrics$procrustes_rmse[[1L]]
    ),
    list(
...
final_rows <- do.call(rbind, lapply(case_results, function(case_result) {
  data.frame(
    ...
    baseline = c("direct_cmd_pure_gmds", "fixed_top_weighted_kk", "phase_e_proxy", "phase_f_shortlist", "oracle_best_rho"),
    method_id = c(
      "cmd_pure_gmds",
...
summary_by_surface <- do.call(rbind, lapply(split(summary_rows, summary_rows$surface), function(df) {
  data.frame(
    ...
    phase_f_beats_phase_e_rho = sum(df$phase_f_beats_phase_e_rho),
    phase_f_beats_fixed_rho = sum(df$phase_f_beats_fixed_rho),
    stringsAsFactors = FALSE
  )
}))
...
summary_lines <- paste(
  ...
  "\\begin{tabular}{lrrrr}",
  "\\toprule",
  "Surface & E = oracle $\\rho$ & F = oracle $\\rho$ & F beats E $\\rho$ & F beats fixed $\\rho$ \\\\",
  "\\midrule",
  paste(
    vapply(seq_len(nrow(summary_by_surface)), function(i) {
      sprintf(
        ...
        summary_by_surface$n_cases[[i]],
        summary_by_surface$phase_f_beats_phase_e_rho[[i]],
        summary_by_surface$n_cases[[i]],
        summary_by_surface$phase_f_beats_fixed_rho[[i]],
        summary_by_surface$n_cases[[i]]
      )
    }, character(1L)),
    collapse = "\n"
...
case_sections <- vapply(case_results, function(case_result) {
  ...
  paste(
    ...
    sprintf(
      paste(
        "This Phase~F case keeps the same seeded MISF-GMDS pipeline but replaces the single-step Phase~E selector by a shortlist selector.",
    ...
    sprintf(
      "\\begin{figure}[p]\\centering\\includegraphics[width=0.98\\linewidth]{%s}\\caption{%s. Final comparison: direct cmdscale control, fixed-top weighted-KK control, Phase~E proxy winner, Phase~F shortlist winner, and oracle best final $\\rho$.}\\end{figure}",
      file.path(pdf_dir, sprintf("%s_final_comparison_grid.png", case_result$case$id)),
      tex_escape(case_result$case$label)
    ),
    sprintf(
      "\\begin{figure}[p]\\centering\\includegraphics[width=0.98\\linewidth]{%s}\\caption{%s. Selected stage layouts for the fixed, Phase~E, Phase~F, and oracle seeded pipelines.}\\end{figure}",
...
tex_lines <- c(
  ...
  "\\title{Phase F: Shortlist Selector Improvement for Seeded MISF-GMDS}",
  "\\author{Selector-improvement study on the mesh-geometry panel}",
  "\\date{2026-04-05}",
  "\\begin{document}",
  "\\maketitle",
  "\\section{Purpose}",
  "Phase~F keeps the current seeded MISF-GMDS pipeline fixed below the top level and studies only the seed selector. The same rectangular mesh-geometry panel is reused: regular $12\\times 12$, regular $15\\times 15$, and irregular rectangle $15\\times 15$, each with flat, saddle, paraboloid, and ripple geometries.",
  "\\section{Protocol}",
  "For each case we compare five corrected top-level seeds (cMDS, KK, GRIP, Weighted GRIP, and Weighted GRIP + polish LGKK). Each seed receives the same short top-level anchor-regularized GMDS correction and pure-GMDS follow-up. We first reproduce the original Phase~E one-level weighted-KK lookahead and its proxy ranking. We then shortlist the top two Phase~E candidates, run one additional weighted-KK insertion/refinement lookahead on those two only, and rank the shortlist by a combined score using the Phase~E rank, second-lookahead stress, second-lookahead roughness, second-lookahead edge-floor, and energy-stability. The final selected pipeline still uses weighted-KK lower-level placement, sparse GMDS refinement, and a short final pure-GMDS polish.",
  "\\section{Block Summary}",
  "The table below aggregates how often the original Phase~E selector and the new Phase~F shortlist selector match the oracle best final $\\rho$ seed, and how often the Phase~F selector improves over the Phase~E selector and the current fixed-top weighted-KK control on final $\\rho$ within each geometry class.",
  summary_lines,
  sprintf(
    "The companion HTML gallery is generated by \\\\texttt{%s}.",
    tex_escape("tools/reports/geodesic_mds_paper/render-gmds-misf-phase-f-shortlist-selector-html.R")
  ),
...
message("Wrote Phase F selection metrics: ", selection_csv)
message("Wrote Phase F shortlist metrics: ", shortlist_csv)
message("Wrote Phase F final metrics: ", final_csv)
message("Wrote Phase F summary metrics: ", summary_csv)
message("Wrote Phase F bundle: ", rds_path)
message("Wrote Phase F report TeX: ", tex_path)
