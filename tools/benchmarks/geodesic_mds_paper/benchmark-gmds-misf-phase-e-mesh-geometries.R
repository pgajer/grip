...
run_tag <- if (smoke) {
  sprintf("gmds-misf-phase-e-mesh-geometries-smoke-%s", format(Sys.Date(), "%Y-%m-%d"))
} else {
  "gmds-misf-phase-e-mesh-geometries-2026-04-02"
}
...
tex_path <- file.path(design_root, "reports", "gmds_misf_phase_e_mesh_geometries_report_2026-04-02.tex")
pdf_path <- file.path(design_root, "reports", "gmds_misf_phase_e_mesh_geometries_report_2026-04-02.pdf")
rds_path <- file.path(tmp_dir, "gmds_misf_phase_e_mesh_geometries_results.rds")
selection_csv <- file.path(tmp_dir, "gmds_misf_phase_e_mesh_geometries_selection_metrics.csv")
final_csv <- file.path(tmp_dir, "gmds_misf_phase_e_mesh_geometries_final_metrics.csv")
summary_csv <- file.path(tmp_dir, "gmds_misf_phase_e_mesh_geometries_summary_metrics.csv")
...
save_candidate_grid <- function(case_result, stage = c("top", "lookahead"), output_path) {
  ...
  ttl <- if (identical(stage, "top")) "Block 1 corrected top-level seeds" else "Block 1 one-level weighted-KK lookahead"
...
case_results <- lapply(cases, function(case) {
  message("Running Block 1 mesh-geometry panel for: ", case$label)
  ...
  selected.ids <- unique(c(proxy.method.id, fixed.method.id, oracle.rho.id))
  ...
  comparison_entries <- list(
    ...
    list(
      ...
      method_label = paste0("Phase E proxy winner (", corrected[[proxy.method.id]]$method_label, ")"),
...
final_rows <- do.call(rbind, lapply(case_results, function(case_result) {
  data.frame(
    ...
    baseline = c("direct_cmd_pure_gmds", "fixed_top_weighted_kk", "phase_e_proxy", "oracle_best_rho"),
    method_id = c(
      "cmd_pure_gmds",
      case_result$fixed_method_id,
      case_result$proxy_method_id,
    ...
    method_label = c(
      ...
      paste0("Phase E proxy winner (", case_result$proxy_method_id, ")"),
    ...
    gmds_stress = c(
      ...
      case_result$candidates[[case_result$proxy_method_id]]$final_metrics$gmds_stress[[1L]],
    ...
    procrustes_rmse = c(
      ...
      case_result$candidates[[case_result$proxy_method_id]]$final_metrics$procrustes_rmse[[1L]],
...
summary_rows <- do.call(rbind, lapply(case_results, function(case_result) {
  data.frame(
    ...
    proxy_matches_oracle_rho = identical(case_result$proxy_method_id, case_result$oracle_rho_method_id),
    proxy_matches_oracle_sigma = identical(case_result$proxy_method_id, case_result$oracle_sigma_method_id),
...
summary_by_surface <- do.call(rbind, lapply(split(summary_rows, summary_rows$surface), function(df) {
  data.frame(
    ...
    n_cases = nrow(df),
    proxy_matches_oracle_rho = sum(df$proxy_matches_oracle_rho),
    proxy_beats_fixed_rho = sum(df$proxy_beats_fixed_rho),
    proxy_beats_fixed_sigma = sum(df$proxy_beats_fixed_sigma),
    stringsAsFactors = FALSE
  )
}))
...
summary_lines <- paste(
  ...
  "\\begin{tabular}{lrrr}",
  "\\toprule",
  "Surface & proxy = oracle $\\rho$ & proxy beats fixed $\\rho$ & proxy beats fixed $\\sigma$ \\\\",
  "\\midrule",
  paste(
    vapply(seq_len(nrow(summary_by_surface)), function(i) {
      sprintf(
        ...
        summary_by_surface$proxy_matches_oracle_rho[[i]],
        summary_by_surface$n_cases[[i]],
        summary_by_surface$proxy_beats_fixed_rho[[i]],
        summary_by_surface$n_cases[[i]],
        summary_by_surface$proxy_beats_fixed_sigma[[i]],
        summary_by_surface$n_cases[[i]]
      )
    }, character(1L)),
    collapse = "\n"
...
case_sections <- vapply(case_results, function(case_result) {
  ...
  paste(
    ...
    sprintf(
      paste(
        "This Block~1 case expands the Phase~E seeded MISF-GMDS comparison beyond paraboloids.",
        "The fixed-top weighted-KK control is \\textbf{%s};",
        "the proxy-selected winner is \\textbf{%s};",
        ...
        "The proxy winner finishes at $\\\\sigma=%s$, $\\\\rho=%s$."
      ),
    ...
    sprintf(
      "\\begin{figure}[p]\\centering\\includegraphics[width=0.98\\linewidth]{%s}\\caption{%s. One-level weighted-KK lookahead used for the truth-free proxy ranking.}\\end{figure}",
      file.path(pdf_dir, sprintf("%s_lookahead_grid.png", case_result$case$id)),
      tex_escape(case_result$case$label)
    ),
    write_candidate_table(candidate.table),
    sprintf(
      "\\begin{figure}[p]\\centering\\includegraphics[width=0.98\\linewidth]{%s}\\caption{%s. Final comparison: direct cmdscale control, fixed-top weighted-KK control, proxy-selected run, and oracle best final $\\rho$.}\\end{figure}",
      file.path(pdf_dir, sprintf("%s_final_comparison_grid.png", case_result$case$id)),
      tex_escape(case_result$case$label)
    ),
    sprintf(
      "\\begin{figure}[p]\\centering\\includegraphics[width=0.98\\linewidth]{%s}\\caption{%s. Selected stage layouts for the fixed, proxy, and oracle seeded pipelines.}\\end{figure}",
...
tex_lines <- c(
  ...
  "\\title{Block 1: Phase E MISF-GMDS Across Mesh Geometries}",
  "\\author{Seeded MISF-GMDS mesh geometry panel}",
  "\\date{2026-04-02}",
  "\\begin{document}",
  "\\maketitle",
  "\\section{Purpose}",
  "Block~1 expands the Phase~E seeded MISF-GMDS study from paraboloids to the broader rectangular mesh geometry panel. The same corrected top-level seed pool is compared on regular and irregular rectangle meshes for four geometries: flat, saddle, paraboloid, and ripple. The current fixed-top weighted-KK control is inherited from the paraboloid study: GRIP on regular $12\\times 12$, Weighted GRIP on regular $15\\times 15$, and cMDS on irregular rectangle $15\\times 15$.",
  "\\section{Protocol}",
  "For each case we compare five corrected top-level seeds (cMDS, KK, GRIP, Weighted GRIP, and Weighted GRIP + polish LGKK). Each seed receives the same short top-level anchor-regularized GMDS correction and pure-GMDS follow-up. We then evaluate a one-level weighted-KK lookahead on the next MISF level and rank the candidates with the same truth-free proxy used in Phase~E. The final selected pipeline uses weighted-KK lower-level placement, sparse GMDS refinement, and a short final pure-GMDS polish.",
  "\\section{Block Summary}",
  "The table below aggregates how often the truth-free proxy matches the oracle best final $\\rho$ seed, and how often it beats the current fixed-top weighted-KK control on final $\\rho$ and $\\sigma$ within each geometry class.",
  summary_lines,
  "The companion HTML gallery is generated by \\texttt{tools/reports/geodesic_mds_paper/render-gmds-misf-phase-e-mesh-geometries-html.R}.",
...
message("Wrote Block 1 selection metrics: ", selection_csv)
message("Wrote Block 1 final metrics: ", final_csv)
message("Wrote Block 1 summary metrics: ", summary_csv)
message("Wrote Block 1 bundle: ", rds_path)
message("Wrote Block 1 report TeX: ", tex_path)
