...
plot_readme_trace_gif <- function(path, trace_obj, edges,
  ...
  # A finished-result preview is also usable without animation or JavaScript.
  file.copy(tail(frame_paths, 1L), sub("[.]gif$", "-poster.png", path), overwrite = TRUE)
  invisible(path)
}
...
# Lightweight poster for the saved overlay; no fits or WebGL are rerun.
magick::image_write(magick::image_read("man/figures/readme-saddle-overlay-rotation.gif")[1],
                    "man/figures/readme-saddle-overlay-poster.png")
...
writeLines(c(
  ...
  paste("R/generator changes relative to commit:",
        system2("git", c("diff", "--quiet", "HEAD", "--", "R", "tools/pkg/generate-readme-assets.R")) != 0L),
  paste("Generator MD5:", unname(tools::md5sum("tools/pkg/generate-readme-assets.R"))),
  paste("grip version:", as.character(utils::packageVersion("grip"))),
  ...
  "man/figures/readme-traces-provenance.txt")
