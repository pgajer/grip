...
dir.create(file.path(out,"fits",key),recursive=TRUE,showWarnings=FALSE)
cache <- function(label,expr) {
 path<-file.path(out,"fits",key,paste0(label,".rds"))
...
for(regime in c("geodesic","ambient")) {
 ...
 for(k in ks) {
  prefix<-paste0(regime,"-k",k);final<-file.path(out,"fits",key,paste0(prefix,if(mode=="extra") "-extra-result.rds" else "-result.rds"))
  ...
  kk<-function(z,uniform=FALSE,extra=FALSE) slim(grip::edge.kk(coords=z,prepared=p,dim=3,max_iter=if(extra) 2000L else if(uniform) 1000L else 200L,
   stiffness_method="density",density_mix_schedule=if(uniform||extra) 1 else c(0,.25,.5,.75,1),scale_mode="profiled",edge_length_epsilon=0,diagnostics=FALSE,return_trace=TRUE,seed=9300000L+replicate))
  ...
  if(mode!="extra") {
   for(nm in names(initial)) for(control in c("primary","uniform")) {
    label<-paste(nm,control,sep="_");fits[[label]]<-cache(paste0(prefix,"-",label),kk(initial[[nm]],control=="uniform"));candidates[[label]]<-fits[[label]]$coords
   }
  } else {
   candidates<-list()
   for(nm in c("classical","stress")) {
    ...
    for(control in c("perturbed","random","original","extended")) {
     label<-paste(nm,control,sep="_")
     z<-switch(control,perturbed=initial[[nm]]+perturb,random=perturb*1e4,original=x,
      extended=readRDS(file.path(out,"fits",key,paste0(prefix,"-",nm,"_primary.rds")))$value$coords)
     fits[[label]]<-cache(paste0(prefix,"-",label),kk(z,extra=control=="extended"));candidates[[label]]<-fits[[label]]$coords
    }
   }
  }
