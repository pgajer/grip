...
grip.optimize.edge.repulsive.stage.R <- function(coords,
  ...
  for (iter in seq_len(max.iter)) {
    ...
    trace[[length(trace) + 1L]] <- data.frame(
      ...
      edge.energy = if (accepted) candidate.state$edge.energy else state$edge.energy,
      repel.energy = if (accepted) candidate.state$repel.energy else state$repel.energy,
      gradient.norm = if (accepted) candidate.state$gradient.norm else state$gradient.norm,
      step = if (accepted) step else NA_real_,
      accepted = accepted,
      n.wall.violations = if (accepted) candidate.state$n.wall.violations else state$n.wall.violations,
      stringsAsFactors = FALSE
    )
    if (!accepted) break
    Z <- candidate
  ...
  list(coords = Z, state = state, trace = do.call(rbind, trace))
}
...
#'
#' @return A list with `coords`, final `state`, and a data-frame `trace`.
#' @export
