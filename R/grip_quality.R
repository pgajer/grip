...
grip.graph.hop.distance.matrix <- function(adj.list) {
  ...
  grip.preflight.preparation(n, sum(lengths(adj.list)) / 2, "distance_matrix_only")
...
grip.prepare.geodesic.kk.base <- function(edges = NULL,
  ...
  grip.preflight.preparation(validated$n, nrow(edges),
    if (caller == "prepare.landmark.geodesic.kk") "landmark_sparse" else "all_pairs")
