...
assessment should match the measurement it uses. Classical MDS assigns each
...
Other analyses follow graph or lineage paths to order observations and study
progression, as in trajectory and pseudotime analysis
[@haghverdi2016; @street2018; @wolf2019]. If an analysis uses or interprets the
length of a path after embedding, fixed-path fidelity assesses whether the path
retains its input-graph length. To make this comparison, we select a shortest
path $\gamma_{ij}$ between vertices $i$ and $j$ in the input graph and keep its
sequence of edges fixed as the coordinates change. We measure the embedded
length of this path by adding the Euclidean distances between the endpoints of
its edges:
...
$d_{ij}^G$. The collection of selected paths is $\Gamma=\{\gamma_{ij}\}$,
and the matrix $D_{G,Z}=(d_{ij}^{G,Z})$ records their embedded lengths. A GMDS
objective measures the discrepancy between these lengths and the corresponding
input graph distances in $D_G$, and seeks coordinates $Z$ that reduce this
discrepancy.
