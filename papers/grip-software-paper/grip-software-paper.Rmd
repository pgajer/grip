...
[@eddelbuettel2011]. Its supported layout API consists of two functions.
`grip()` returns an $n \times m$ numeric coordinate matrix, whereas `trace.grip()` returns the
...
```{r mobius-dimension, echo=FALSE, fig.cap=sprintf("Direct three-dimensional layout and a four-dimensional layout projected to three dimensions for the same 300-vertex Möbius-strip graph. Both use weighted.grip.nd() with unit edge lengths, the same refinement settings, and seed 1. The four-dimensional coordinates are projected onto their first three principal components, retaining %.1f%% of the 4D coordinate variance. For display, both configurations are centered, scaled to unit root-mean-square radius, orthogonally aligned to a common reference, and viewed from the same direction. Thicker blue edges mark the strip boundary. This example illustrates the layout-and-projection workflow rather than reproducing the historical experiment or establishing improved graph-distance fidelity.", 100 * mobius.example$retained.variance), fig.alt="Side-by-side Möbius-strip drawings show a direct three-dimensional layout and a four-dimensional layout projected to three dimensions by PCA. The projected drawing has a subtly smoother twist. The thicker boundary curve is blue in both panels.", fig.width=8, fig.height=3.5, fig.pos="H"}
mobius_draw_pair(mobius.example)
```

Both core functions use the `metric` argument to specify the graph geometry.
...
generators provide examples, regression fixtures, and controlled benchmarks;
they are not substitutes for data-driven graph construction.
...
The matrix $D_{G,Z}=(d_{ij}^{G,Z})$ contains their embedded
...
The angle example in Figure \@ref(fig:path-angle-freedom) shows why fixed-path
fidelity alone is insufficient for chord-based readings. Accordingly, `score.gmds()`
reports chord and fixed-path diagnostics on the same
prepared graph without combining them into a single score.

## Method families and function entry points

Table \@ref(tab:family-scope) maps the supported multiscale layout API and the
experimental GMDS families to their computational roles and principal functions.
...
passed to a GMDS-facing refinement method.
...
The `edge.kk()` function returns refined coordinates and a diagnostic panel.
...
generated directly in the paper build.
...
seeds. The result is family-specific evidence for the supplied defaults, not a
...
metric-MDS and weighted-GRIP configurations. Six LGKK polish rounds provide a
...
use seed 1. The graph and one selected shortest path for each of its 300
...
coords.weighted.edge <- weighted.saddle$layouts[["Weighted GRIP + edge-KK"]]
coords.mds.edge <- weighted.saddle$layouts[["Metric MDS + edge-KK"]]
...
accessible_kable(
  ...
  caption = paper_table_caption(
    paste("Three-dimensional weighted saddle-surface layouts, scored on the",
          "same 40 edges and 300 fixed paths with separately profiled scales."),
  ...
  alt = paste(
    ...
    "edge-KK refinement of the latter two, LGKK-polished weighted GRIP,",
    "and weighted igraph Kamada-Kawai. Both edge-KK refinements reduce",
    "edge and fixed-path relative RMSE below one billionth."
  ),
  digits = 3
)
```
...
$10^{-9}$ in this example. The unrefined metric-MDS and weighted-GRIP rows show
...
efficiency or independent validation of the graph geometry.
...
```{r weighted-figure, fig.cap="Three-dimensional weighted mesh layouts after similarity alignment to the target, displayed with a common orthographic projection and scale. The top row shows the target geometry, weighted GRIP, and weighted GRIP followed by edge-KK; the bottom row shows metric MDS followed by edge-KK, weighted GRIP followed by six LGKK polish rounds, and weighted igraph KK. Both weighted-GRIP refinements start from the coordinates shown in the top-center panel. Near-zero edge and fixed-path loss need not recover the target's extrinsic curvature.", fig.alt="Six consistently projected panels show three-dimensional layouts of the same weighted saddle mesh: target, weighted GRIP, weighted GRIP plus edge-KK, metric MDS plus edge-KK, weighted GRIP plus LGKK, and weighted igraph Kamada-Kawai. Low-loss refinements need not reproduce the target's saddle shape.", fig.width=11.2, fig.height=6.8, fig.pos="H"}
weighted.layouts.aligned <- list(
  ...
  "Weighted GRIP + edge-KK" = align_to_reference(
    coords.weighted.edge, weighted.mesh$coords_surface
  ),
  "Metric MDS + edge-KK" = align_to_reference(
    coords.mds.edge, weighted.mesh$coords_surface
  ),
...
weighted.colors <- c("#666666", "#1F3B73", "#1F3B73", "#684A88", "#7A2E2E", "#3A6B35")

op <- par(mfrow = c(2, 3), mar = c(1, 1, 3, 1), cex = 1.1)
...
The bundled benchmark applies a common scoring function to
...
The core GRIP layout implementation is single-threaded; selected experimental
GMDS refinements can use up to two compiled-engine threads.
...
The GMDS-facing methods provide tools for investigating a supplied graph
