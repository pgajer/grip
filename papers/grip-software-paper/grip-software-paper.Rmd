...
`weighted.grip.nd()` and selected GMDS methods support any $m\geq 2$.
...
The appropriate diagnostic depends on the analysis. Proximity measures,
neighborhood and clustering analyses, and distances to centroids in the
embedded coordinates use endpoint chords. Trajectory and pseudotime analyses
instead use graph or lineage paths to order cells and follow changes along
those paths [@haghverdi2016; @street2018; @wolf2019]. Chord fidelity governs
the former; fixed-path fidelity governs the latter.

```{r noisy-circle-path-geometry, fig.cap="A graph-to-embedding contrast for the identity embedding of a data-derived graph. Left: 120 observations sampled around a unit circle with truncated Gaussian radial offsets. Right: their connected symmetric 6NN graph drawn at the same coordinates, so $z_i=x_i$. The white endpoints are joined by one fixed graph shortest path (blue), whose embedded length exactly equals its input-graph length, and by their Euclidean chord (dashed red); the legend gives their lengths. The display compares two metrics on the same graph and embedding; it does not establish that a chosen neighborhood graph recovers latent geometry.", fig.alt="Two panels show a noisy circular point cloud and its symmetric six-nearest-neighbor graph. In the graph panel, which uses the identity embedding, two highlighted endpoints are joined by a thick blue graph shortest path around the circle and a dashed red Euclidean chord across the center. The graph path retains its input-graph length and is longer than the chord.", fig.width=10, fig.height=4.9, fig.pos="H"}
...
fidelity alone is insufficient for analyses based on endpoint chords. Accordingly, `score.gmds()`
reports chord and fixed-path diagnostics on the same
prepared graph without combining them into a single score.

## Method families and corresponding functions

Table \@ref(tab:family-scope) summarizes the multiscale layout methods and
experimental GMDS methods, their purposes, and the corresponding functions.
...
accessible_kable(
  ...
  function_columns = 4L,
  caption = "Status, purpose, and principal functions of the multiscale layout and GMDS methods in grip. Exp. = experimental.",
  alt = paste(
    "Six rows distinguish the standard multiscale GRIP methods from the",
...
and $d_p^Z$ its Euclidean layout distance. The function fits the scale
...
alternative for larger weighted graphs. The R example below prepares the graph,
scores the initial coordinates, and refines them.
...
unintended changes in results. Table \@ref(tab:preset-benchmark) illustrates
their behavior on two controlled structured families, each run across ten
seeds. The results are calculated by the R code accompanying the article.
...
claim that one choice of settings is preferable for arbitrary graphs. Figure
...
accessible_kable(
  ...
  row.names = FALSE,
  align = c("l", "r"),
  caption = "Construction summary for the UMB-HMP-only Illumina 16S graph provided with the package.",
  alt = paste(
...
The comparison applies the same scoring function to
...
`r benchmark.results$benchmark_metadata$platform`, and R's reference Basic Linear
Algebra Subprograms (BLAS) library. The supplementary computing-environment
record gives the BLAS library's location relative to R's installation directory.

## Computational boundaries

The multiscale GRIP implementation uses a single thread of execution; selected
experimental GMDS refinements can use up to two threads for parallel computation.

The example uses the full 4,391-vertex giant component of the
...
benchmark of the package on larger graphs. The original GRIP studies
...
with the searches without early stopping used for coarsest-level initialization. For
...
refinement. With fixed limits on retained neighbors, geometrically decreasing
hierarchy sizes also yield $O(n)$ peak memory use under the same bounded-degree assumption.
...
The optional `metric_neighbor_cap` limits the number of vertices whose shortest
distances are finalized during
...
diagnostics and refinement without storing shortest paths for all vertex pairs. Metric MDS,
...
layout computation. Assessing these costs on larger graphs requires a separate,
systematically designed study of GMDS methods.
