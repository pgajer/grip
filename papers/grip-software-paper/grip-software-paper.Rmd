...
is not. A $10 \times 10$ mesh whose edge lengths are induced by a curved saddle
...
```{r weighted-figure, fig.cap="Three-dimensional layouts of a 10 by 10 weighted saddle mesh after similarity alignment to the target, displayed with a common orthographic projection and scale. The top row shows the target saddle, metric MDS, and metric MDS followed by edge-KK; the bottom row shows weighted igraph KK, weighted GRIP, and weighted GRIP followed by edge-KK. In each row, the right-hand layout starts from the coordinates in the middle panel. Low edge and fixed-path loss need not recover the target's extrinsic curvature.", fig.alt="Six consistently projected panels show three-dimensional layouts of a 100-vertex saddle mesh. The middle and right columns compare metric MDS and weighted GRIP before and after edge-KK. The left column contains the target saddle and weighted igraph Kamada-Kawai. The refined weighted-GRIP layout remains visibly flatter than the target.", fig.width=11.2, fig.height=6.8, fig.pos="H"}
