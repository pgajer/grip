...
notion of fidelity. Both compare against the input graph distances $D_G$, but
chord criteria measure Euclidean endpoint distances, whereas fixed-path
criteria measure lengths along retained graph paths after embedding.
Isomap uses $D_G$ because ambient straight-line
...
the former; fixed-path fidelity governs the latter. The worked example
\emph{Linking the two fidelity stages} below compares these criteria after
calibrating graphs against a known saddle surface.
...
Known surface geometry allows graph construction to be assessed before an
embedding is evaluated. We drew five independent samples of 1,000 points,
uniformly by surface area, from the saddle
$z=0.8(x^2-y^2)$ over $(x,y)\in[-1,1]^2$. For each sample,
...
```{r saddle-calibration-layouts, fig.cap="Graph calibration and representative three-dimensional configurations for five independent saddle samples. A: surface-to-graph relative RMSE over the tested neighborhood sizes. B: enlargement near the minima, with a different vertical scale. Open circles mark within-sample minima. C--E: original coordinates, metric MDS, and metric MDS followed by edge-KK for cloud 5 (n = 1,000; k = 73), selected by median minimum surface-to-graph error, not by appearance. The layouts are similarity-aligned to the generating coordinates and shown with one projection and scale; vertex colors retain original x-coordinate identity. Faint gray lines show graph edges. Blue marks the same retained graph path, and dashed brown its endpoint chord. Fidelity scores are computed before display alignment and projection.", fig.alt="Two calibration plots show surface-to-graph errors across five samples, followed by three consistently aligned 3D layouts of the median-selected sample: original saddle, metric MDS, and metric MDS refined by edge-KK.", fig.width=10.6, fig.height=7.1, fig.pos="H"}
...
```{r saddle-pilot-scores, fig.cap="Paired graph-to-embedding comparisons across five independent samples of 1,000 observations. Each small dot represents one sample; thin lines join results on the same selected graph. Open circles and numeric labels show medians. All configurations are three-dimensional. Fixed-path and chord scores use all 499,500 unordered pairs, while edge scores use all graph edges. The three statistics have different normalizations, and vertical scales differ. These fixed-budget results include all five samples; no confidence intervals are shown.", fig.alt="Three panels compare original saddle coordinates, metric MDS, and MDS followed by edge-KK across five samples. Edge-KK lowers fixed-path and edge errors in every sample but increases chord stress.", fig.width=10.2, fig.height=3.8, fig.pos="H"}
plot_pilot_scores(saddle.pilot)
```
...
The fixed-grid saddle comparison in Supplement S3 provides a complementary
example with a prescribed graph. It includes weighted-GRIP initialization,
edge-KK refinement of both weighted-GRIP and metric-MDS coordinates, and LGKK
refinement. Its horizontal and vertical mesh edges define the graph metric;
unlike the sampled-surface example above, it does not assess approximation to
continuous-surface geodesics.
