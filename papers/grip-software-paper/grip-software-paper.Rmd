...
abstract: >
  ...
  the resulting geometry may not faithfully preserve distances measured along
  shortest paths in the graph. When applied to graph distances, multidimensional
  scaling (MDS) assesses how closely straight-line distances between embedded
  vertices approximate graph shortest-path distances. Graph-geodesic
  multidimensional scaling (GMDS) instead assesses whether selected shortest
  paths retain their lengths after embedding, measuring each embedded path by
  summing the Euclidean lengths of its edges. grip is an R package for generating
  graph layouts, assessing them under both criteria, and refining them through
  experimental GMDS-oriented workflows.
preamble: |
  \usepackage{amsmath}
  \usepackage{graphicx}
  \usepackage{xcolor}
...
Multidimensional scaling (MDS) seeks a low-dimensional Euclidean representation
of a set of objects in which the distances between points approximate the input
dissimilarities between the corresponding objects. Given a dissimilarity matrix
$\Delta=(\delta_{ij})$, MDS seeks coordinates
$Z=\{z_1,\ldots,z_n\}\subset\mathbb{R}^m$ such that the pairwise Euclidean
distances $D_Z=(d_{ij}^Z)$, with $d_{ij}^Z=\lVert z_i-z_j\rVert_2$,
approximate the corresponding entries of $\Delta$
[@kruskal1964multidimensional]. The choice of dissimilarities supplied to MDS
becomes particularly important when the observations
$X=\{x_1,\ldots,x_n\}\subset\mathbb{R}^p$ lie on a curved, branching, or
otherwise non-Euclidean structure. In such settings, straight-line distances in
the ambient space may poorly represent the intrinsic distance structure
$D_X=(d_{ij}^X)$ of the data. Isomap addresses this problem by constructing a
weighted neighborhood graph $G=(V,E,\ell)$, whose vertices correspond to the
observations, and using the graph shortest-path distance matrix $D_G=(d_{ij}^G)$
to approximate $D_X$. Classical MDS is then applied to $D_G$ to obtain a
low-dimensional embedding $Z$ [@tenenbaum2000]. This $X\to G\to Z$
construction involves two distinct questions of fidelity: how well does $D_G$
approximate the intrinsic distances $D_X$, and how faithfully does the embedding
$Z$ preserve the graph distances $D_G$?

Data-derived graphs can also provide neighborhoods for regression or
classification of an observed response [@belkin2006; @kim2016]. In these
settings, graph construction can be assessed by predictive performance rather
than geometric fidelity alone. Once a graph has been chosen, its embedding may
serve as a visualization or provide distances for subsequent calculations. The
companion \CRANpkg{dgraphs} package addresses the $X\to G$ stage, whereas
\CRANpkg{grip} addresses the $G\to Z$ stage. The diagram below summarizes the
two stages and their geometric fidelity questions.
...
At the $G\to Z$ stage in the diagram, we can measure distances directly between
embedded vertices or along the graph paths that connect them. When an analysis
uses these embedded distances in place of input graph distances, its fidelity
assessment should match the measurement it uses. MDS assigns each
vertex a point in Euclidean space, giving the coordinate map
$\phi:V\to\mathbb{R}^m$ and configuration $Z=\phi(V)\subset\mathbb{R}^m$.
It places the points so that their straight-line distances, also called
endpoint chords, $d_{ij}^{Z}=\lVert z_i-z_j\rVert_2$, approximate the graph
distances $d_{ij}^G$. Chord fidelity measures how closely these distances agree.

Following a path through the graph measures a different quantity from the
straight-line distance between its endpoints. Graph and lineage paths are used,
for example, to order observations and study progression in trajectory and
pseudotime analysis [@haghverdi2016; @street2018; @wolf2019]. When an analysis
uses or interprets a path's length after embedding, fixed-path fidelity measures
how closely that length matches its original graph length. For each pair of
vertices $i$ and $j$ being compared, we select a
shortest path $\gamma_{ij}$ in the input graph. We retain the same sequence of
edges as the coordinates change. Along each edge $(u,v)$, we measure the
straight-line distance $\lVert z_u-z_v\rVert_2$ between its embedded endpoints.
Adding these edge lengths gives the embedded path length:
$$
d_{ij}^{G,Z}
=
...
\noindent We compare this sum with the path's original graph length
$d_{ij}^G$. The retained paths form the collection $\Gamma=\{\gamma_{ij}\}$,
and their embedded lengths form the matrix $D_{G,Z}=(d_{ij}^{G,Z})$. A GMDS
objective compares these embedded lengths with the original graph distances in
$D_G$. Minimizing this objective seeks coordinates $Z$ that reduce the
discrepancy for the fixed collection of paths.
...
`edge.kk(init = "classical_mds")` uses classical MDS as an initializer before
...
```{r panel-e-workflow-output, echo=FALSE, fig.cap="Representative three-dimensional configuration produced by the metric-MDS + edge-KK workflow (n = 1,000; k = 73). Colors encode the original x coordinate, from blue to brown. The fixed Delaunay triangulation of the original parameter coordinates is carried to the embedding to show its surface structure; it is a visualization mesh, not the skNN graph. Coordinate axes identify the displayed orientation. Coordinates are similarity-aligned to the generating observations for display, after scoring. No retained path or endpoint chord is superimposed; the annotated comparison appears in the worked example below.", fig.alt="Large three-dimensional view of the metric-MDS plus edge-KK saddle configuration with colored points, a translucent triangular mesh, and labeled x, y, and z axes.", out.width="100%", fig.align="center", fig.pos="H"}
...
## Application to a real microbiome network

```{r hmp-data, include=FALSE}
data(hmp.gc)
hmp.edges <- edge.matrix.from.adj(hmp.gc$adj_list)
hmp.gmds <- read.extdata.rds("hmp_gmds", "hmp_gmds_layout.rds")
```

The controlled saddle example assesses graph construction against a known
surface before evaluating an embedding. This example instead illustrates the
$G\to Z$ workflow on a real biological graph, for which the underlying data
geometry $D_X$ is unknown. The package includes a graph derived from the
University of Maryland Human Microbiome Project (UMB-HMP), a longitudinal study
of 135 nonpregnant women of reproductive age who self-collected vaginal swabs
daily for ten weeks [@ravel2013daily].

The graph is constructed from 4,411 Illumina 16S rRNA profiles. The 169
taxonomic features detected in at least 1% of samples are converted to relative
abundances and represented by ten principal components. A symmetric $k=3$
nearest-neighbor graph assigns Euclidean distances in this representation as
edge lengths. Its largest connected component contains 4,391 vertices and
9,067 edges.

\Needspace{8\baselineskip}
Figure \@ref(fig:hmp-layouts) applies the GMDS-oriented sequence introduced
above: metric-MDS embeds the weighted graph shortest-path distances, and
edge-KK then refines the configuration using the supplied edge lengths. The
profiled edge relative RMSE decreases from
`r sprintf("%.1f", 100 * hmp.gmds$metric_mds_diagnostics$edge.rel.rmse)`% to
`r sprintf("%.1f", 100 * hmp.gmds$edge_kk_diagnostics$edge.rel.rmse)`%.
Community-state-type (CST) labels are used only to color the vertices and were
not supplied to either layout method.

```{r hmp-layouts, fig.cap="Weighted layouts of the 4,391-vertex UMB-HMP graph provided with the package. Metric-MDS (left) embeds graph shortest-path distances; edge-KK (right) refines that configuration using the supplied edge lengths. Vertices are colored by community-state type only for interpretation; these labels are not layout inputs. Each configuration is centered and scaled independently for display.", fig.alt="Two panels show the same 4,391-vertex UMB-HMP graph with vertices colored by seven community-state-type labels. The left panel is metric-MDS and the right panel is the resulting metric-MDS plus edge-KK layout.", fig.width=10, fig.height=4.5, fig.pos="H"}
...
hmp.layouts <- list(
  "metric-MDS" = hmp.gmds$metric_mds,
  "metric-MDS + edge-KK" = hmp.gmds$metric_mds_edge_kk
)
...
The central premise of this article is that a data-derived graph embedding has
two linked but distinct fidelity problems. The $X\to G$ stage asks whether
graph shortest-path distances represent the geometry of the data, whereas the
$G\to Z$ stage asks whether the coordinates preserve the relevant features of
the supplied graph geometry. The saddle experiment makes this separation
concrete: graph construction was calibrated against known surface geodesic distances
before the coordinates were assessed. Good $G\to Z$ agreement cannot recover
geometry already lost during $X\to G$.

At the $G\to Z$ stage, fidelity depends on which geometric quantities matter
for the analysis. In the saddle experiment, edge-KK refinement reduced edge
and fixed-path errors but increased MDS Stress-1. This tradeoff is expected:
edge-KK targets edge-length residuals, and although zero edge loss guarantees
zero fixed-path loss, away from zero the edge and path objectives can have
different minimizers and neither directly minimizes chord stress. Coordinate
agreement and surface agreement also need not change together. These measures
are therefore complementary descriptions of an embedding rather than
interchangeable measures of quality. The criterion used to select a
configuration should reflect the distances used in the downstream analysis.

Uncertainty in graph construction remains part of the interpretation. When a
synthetic calibration family leaves several graph constructors or parameter
settings plausible, the $G\to Z$ assessment should be repeated across that set
rather than conditioned on a single nominal choice. Its relevance to real data
also depends on how well the calibration family represents the topology,
sampling process, and noise structure of the application.

The $X\to G$ construction need not begin with Euclidean distances. Given a
...
supplied by the first stage, but not the subsequent definitions of $D_G$,
$D_{G,Z}$, or the $G\to Z$ fidelity criteria.

Computational scale introduces a separate choice among full, landmark,
edge-only, and multiscale strategies. When all-pairs distances are feasible,
metric-MDS followed by edge-KK provides a practical GMDS-oriented sequence; for
larger graphs, weighted GRIP can supply the initial coordinates without the
dense classical-scaling step. The package does not select automatically among
these strategies. Moreover, the five sampled saddle point clouds and the single
UMB-HMP graph do not establish a general ranking or empirical scaling law for
embedding methods.

A broader evaluation should cross graph constructors and their settings with
embedding objectives and algorithms, using multiple realizations from several
controlled data families. Such a design can reveal which conclusions are
stable to uncertainty in the graph rather than optimal for one selected
construction. \CRANpkg{grip} provides the layout, scoring, and refinement
functions needed for these comparisons while keeping the two fidelity stages
explicit.
