...
knitr::read_chunk("reproducibility/scripts/edge-kk-workflow.R")
...
`score.layout()` provides general layout diagnostics, while `compare.layouts()`
applies these diagnostics across repeated candidate layouts.
...
Eight refinement iterations reduce the profiled edge relative RMSE from
`r sprintf("%.3f", before$edge.rel.rmse)` to
`r sprintf("%.3f", after$edge.rel.rmse)`.
Supplement S3 shows the corresponding configurations.
