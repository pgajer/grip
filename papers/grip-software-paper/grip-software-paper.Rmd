...
In differential geometry, a Riemannian isometric embedding
$f:(M,g)\rightarrow\mathbb{R}^m$ preserves the length of every piecewise
smooth curve $\gamma:[0,1]\rightarrow M$:

$$
L_g(\gamma)=L_{\mathrm{Euc}}(f\circ\gamma).
$$

\noindent Here the left-hand side measures length using the Riemannian metric
$g$, while the right-hand side measures Euclidean arc length along the image
curve. This equality does not require the curve's length to equal the
straight-line distance between its endpoints [@saucan2012]. In contrast, zero
MDS Stress-1 for intrinsic input distances $d_M(x,y)$, measured along $M$,
requires $d_M(x,y)=\lVert f(x)-f(y)\rVert_2$ for every evaluated pair.
