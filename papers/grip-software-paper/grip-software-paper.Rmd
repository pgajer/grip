...
Preserving lengths along an embedded object does not require those lengths to
equal straight-line distances through the surrounding space. In differential
geometry, a Riemannian isometric embedding
$f:(M,g)\rightarrow\mathbb{R}^m$ preserves the length of every curve on $M$.
It therefore preserves intrinsic distances $d_M(x,y)$, defined as the infimum
of lengths of curves joining $x$ and $y$ on $M$, when distances in the image
are also measured along curves confined to $f(M)$. The Euclidean chord can
leave $f(M)$ and be shorter [@saucan2012]. Exact chord preservation would
additionally require $d_M(x,y)=\lVert f(x)-f(y)\rVert_2$ for every pair;
preserving curve lengths alone does not ensure this equality. For a graph,
fixed-path relative RMSE compares each selected input path's original length
with the sum of the Euclidean lengths of its edges after embedding. It asks
whether these retained paths preserve their lengths, not whether the chords
joining their endpoints have those lengths.
