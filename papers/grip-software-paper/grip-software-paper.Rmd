...
$Z$ preserve the graph distances $D_G$? The companion \CRANpkg{dgraphs} package
addresses the $X\to G$ stage, whereas \CRANpkg{grip} addresses the $G\to Z$
stage. These two stages are summarized below.
...
Beyond approximating intrinsic distances, data-derived graphs can provide
neighborhoods for regression or classification of an observed response
[@belkin2006; @kim2016]. In these settings, graph construction can be assessed
by predictive performance rather than geometric fidelity alone. Once a graph
has been chosen, its embedding may serve as a visualization or provide distances
for subsequent calculations. If an analysis uses the original graph and its
weights, changing only the displayed coordinates does not alter its results.

When distances measured in an embedding replace input graph distances, the
relevant assessment depends on how those embedded distances are measured.
...
Chord fidelity assesses this endpoint-distance approximation. Fixed-path
fidelity instead concerns calculations or interpretations based on lengths
along retained graph paths. Graph and lineage paths are used to order
observations and study progression in trajectory and pseudotime analysis
[@haghverdi2016; @street2018; @wolf2019]. If a selected input path's length is
measured after embedding, the relevant comparison is whether its embedded length
