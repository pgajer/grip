...
Supplement S3 lists the method families and their principal functions.
...
The \pkg{ivue} call below creates the interactive view corresponding to panel E
(Supplementary Figure S4.1C). The supplied helper `panel_e_display()` applies
the panel's similarity alignment, colors, and common display bounds.
...
skNN graph. The blue path and brown endpoint chord illustrate the two distance
measurements. Alignment is for display only, after scoring.

```{r panel-e-view, echo=TRUE, eval=FALSE}
```

The complete runnable example is
`reproducibility/scripts/panel-e-workflow.R`; the visualization requires
\pkg{ivue} and \CRANpkg{rgl}. Supplement S3 retains a smaller mesh example
that illustrates edge-KK without the visualization dependencies.
