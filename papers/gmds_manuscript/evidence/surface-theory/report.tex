...
converges to a four-branch tree metric: its intrinsic dimension is one, yet
its classical Euclidean approximation spans three dimensions. This nonplanar
limit is proved for generic samples covering all four branches. Selected
raw-stress fits support the same behavior over the tested radii, without
establishing convergence or universal nonplanarity. Experiments use
...
The main text includes the comparative shape figure. The supporting atlas uses
larger pages where needed to preserve readable labels; each figure is linked
...
\begin{itemize}
 ...
 \item \textbf{Saddle, geodesic input.} Across the tested large radii, selected
 embeddings develop four thin branches spanning three dimensions. The classical-MDS nonplanarity
...
The strongest geometric distinction is between \textbf{intrinsic dimension
and Euclidean span}. The saddle's limiting metric is a one-dimensional tree,
but its classical MDS approximation has three independent Euclidean directions.
Branches can be individually thin while the whole configuration is nonplanar.
Figure~\ref{fig:scompare} in the main text compares this behavior with the
paraboloid's line limit.
...
\citep{rcmdscale}. In grip 0.2.0, \texttt{metric.mds()} called that classical
calculation on its prepared graph-distance matrix. From 0.2.0.9000 it is named
\texttt{classical.mds()}, while \texttt{metric.mds()} wraps ratio SMACOF. The present
...
The bounded-stress construction is an asymptotic argument, not a sharp
finite-radius estimate. In the present interior sample the smallest positive
squared-radius gap is about $6.06\times10^{-6}$. The explicit feasible
candidate's unnormalized stress still rises from about $6.87\times10^6$ at
$r=64$ to $1.32\times10^8$ at $r=1024$. This is consistent with eventual
$O(1)$ boundedness: the constants depend strongly on the fixed radial gaps,
and the tested radii need not have reached that regime.
...
To see the unequal-radius correction directly, set $a=r\sqrt p$,
$b=r\sqrt q$, and $h=\log(b/a)$ for fixed $0<p<q$. At sufficiently large
$r$, a bounded angular separation uses the monotone-radial geodesic branch.
For its Clairaut constant $c$, the angular and length integrands expand as
\begin{align}
 \frac{c\sqrt{1+4\rho^2}}{\rho\sqrt{\rho^2-c^2}}
 &=\frac{2c}{\rho}+O(\rho^{-3}),\\
 \frac{\sqrt{1+4\rho^2}}{\sqrt{1-c^2/\rho^2}}
 &=2\rho+\frac{c^2+1/4}{\rho}+O(\rho^{-3}).
\end{align}
Integration gives $\alpha=2ch+O(r^{-2})$, hence
$c=\alpha/(2h)+O(r^{-2})$, and
\begin{equation}
 g(r)=b^2-a^2+(c^2+1/4)h+O(r^{-2})
     =b^2-a^2+\frac h4+\frac{\alpha^2}{4h}+O(r^{-2}).
\end{equation}
Since $h=\tfrac12\log(q/p)$, this is the stated correction. Squaring gives
\eqref{eq:c-matrix}. The expansion is uniform for $c$ in a fixed bounded
neighborhood and for the particular fixed positive radial levels; it is not
a uniform estimate over growing samples with arbitrarily close radii.
...
\end{equation}
For each of the two sampling measures studied, numerical quadrature of the
second-order operator identifies the leading transverse modes as the sine
and cosine modes with $m=1$. Dominance over other modes is a numerical finding,
not an analytic mode-selection theorem. Their radial component solves
...
The competing sectors can be derived explicitly. Let $\mathcal L$ be the
integral operator with kernel $\ell(q,p)$ in $L^2(\mu)$, and let
$\mathcal H$ project off $\operatorname{span}\{1,q\}$ in that space. The radial
logarithmic term in \eqref{eq:c-matrix} is
\begin{equation}
 R(q,p)=\tfrac14\{q\log q+p\log p-q\log p-p\log q\}.
\end{equation}
Each summand has a factor $1$ or $q$ in at least one argument. Consequently
$\mathcal H R\mathcal H=0$. The remaining angular average gives the radial
sector $-\pi^2\mathcal H\mathcal L\mathcal H/6$. In nonzero Fourier sector $m$,
the cosine/sine orthogonality factor $1/2$ combines with the coefficient in
the Fourier series and classical MDS's factor $-1/2$ to give
\begin{equation}\label{eq:angular-sectors}
 \mathcal K_m=\frac{(-1)^{m+1}}{m^2}\mathcal L,\quad m\ge1;
 \qquad \mathcal K_0=-\frac{\pi^2}{6}\mathcal H\mathcal L\mathcal H.
\end{equation}
For $m\ge1$, the constant and height projections vanish by angular
orthogonality. This accounts for the signs and the competing-sector checks.
...
Table~\ref{tab:population-sectors} records the competing eigenvalues at 256
nodes, including the second eigenvalue within the $m=1$ radial problem.
Every competitor is far below the leading eigenvalue for each measure.
The same ordering was checked at 512 nodes. These checks support the selected
pair without constituting a rigorous bound on quadrature error.
\input{tables/population_sectors.tex}
...
Figures~\ref{fig:s3d}--\ref{fig:sshape} and the main-text comparison in
Figure~\ref{fig:scompare} show that the selected three-dimensional MDS outputs
develop four narrow branches over the tested radii. Their three singular values
remain comparable. This is finite-radius evidence for raw stress, not a proof
that its selected fits converge. The raw-stress base-disk results are:
...
\citep[Theorem 6.5.2, Corollary 6.5.3, and proof of Lemma 6.5.4,
pp.~124--125]{gorodski2022}.
...
A readily checked sufficient condition is that all four branches contain a
nonroot point and one branch contains two distinct positive lengths $t_i$.
A sampled root also suffices when all four branches are represented.
To verify either condition, suppose $JFv=0$. Then $Fv$ is constant. On one
branch with two different positive lengths, constancy forces its component
of $v$ and the constant to be zero; a sampled root forces the constant to zero
directly. One nonroot point on each remaining branch then forces all other
components of $v$ to vanish, proving rank four.
...
\subsection{Optimizer spread and a direct test of the saved planar fits}
Optimizer success flags do not describe how much the fitted solutions depend
on initialization. Table~\ref{tab:optimizer-spread} compares the six original
3D starts at $r=64$. The relative spread is $(S_{\max}-S_{\min})/S_{\min}$;
division by pair count or RMS target scale cancels in this ratio.
\input{tables/optimizer_spread.tex}
The saddle starts agree essentially to numerical precision, whereas the
paraboloid starts reach materially different local minima. Sixteen additional
paraboloid runs---eight for each sampling measure, comprising a selected-fit
restart, three perturbations, and four new random starts---improved the
selected stress only at relative order $10^{-9}$. These checks support the
selected fits but do not certify global optima. The variability across
paraboloid starts is another reason to treat its quadratic coefficients as
descriptive rather than universal geometric constants.

The saved saddle planar solutions admit a more direct dimensionality check.
Normalize targets and coordinates by RMS input distance and use mean pair
stress $F=S/M$. At planar coordinates $x_i\in\R^2$ with all $d_{ij}>0$, add
a third coordinate $\varepsilon z_i$. A Taylor expansion gives
\begin{equation}
 F(x,\varepsilon z)=F(x,0)+\frac{\varepsilon^2}{M}
 \sum_{i<j}\frac{d_{ij}-\Delta_{ij}}{d_{ij}}(z_i-z_j)^2+O(\varepsilon^4).
\end{equation}
Thus the Hessian in the added coordinates is $2L_w/M$, where
$w_{ij}=(d_{ij}-\Delta_{ij})/d_{ij}$ and
$L_w=\diag(\sum_j w_{ij})-w$ is the weighted Laplacian (weights can be negative).
At $r=64$, its smallest eigenvalues are approximately $-0.0034782$ for
base-disk sampling and $-0.0036110$ for surface-area sampling. Perturbations
of size $10^{-3}$ along unit eigenvectors decrease the saved mean stresses;
finite differences at size $10^{-5}$ agree with the negative curvatures to
within $2\times10^{-6}$ in absolute value. The two saved planar fits are
therefore not 3D local minima. This excludes those configurations, not all
other planar stationary points or every possible planar global minimum.
...
For the saddle, 60 validation comparisons (12 endpoint pairs at five radii)
were checked using independent SciPy DOP853 integration and boundary-value
collocation. The maximum relative length disagreement \emph{within that
validation sample} was $7.87\times10^{-9}$ or less; maximum independent
...
internal shooting residual. The maximum relative energy drift on the
\emph{independently integrated DOP853 trajectories} was less than
$6.32\times10^{-11}$; this is not a drift measurement of saved production
trajectories. Exact straight rulings and reflection-axis meridian
integrals provide additional checks.

\textbf{These maxima are not all-pairs accuracy bounds.} A targeted follow-up
check selected 24 additional pairs from each actual $r=64$ matrix: the four
shortest distances, four longest distances, four pairs with an endpoint nearest
zero height, and twelve new random pairs. All 48 comparisons completed using
independent DOP853 integration and collocation. Eight initial collocation
attempts at tolerance $10^{-8}$ exhausted the node limit; retries at
$10^{-7}$ succeeded. The three most discrepant pairs were then checked at
three tighter settings, through collocation tolerance $10^{-10}$.

The largest refined relative disagreement was approximately
$8.48\times10^{-8}$, for surface-area sample indices 58 and 107 (zero based).
The saved distance is $60.15202505808$ and the refined value is
$60.15201995534$, an absolute discrepancy of about $5.10\times10^{-6}$.
Relative disagreements here use the saved pair's own distance as denominator,
which is useful for short pairs. This discrepancy is still small compared
with the reported MDS fitting errors, but it demonstrates why a maximum over
a validation sample must not be stated as an error bound for every pair.
...
and classical three-dimensional counterparts, within numerical tolerance.
...
\end{quote}
The targeted follow-up checks reproduce the additional geodesic comparisons,
all-radius optimizer spreads, sixteen extra paraboloid starts, transverse
Hessians, and the population-sector eigenvalues. They read the saved experiment
outputs and do not replace their matrices, coordinates, or selected fits.
The report build verifies these diagnostic results as well as the original
experiment bundles, checksums its consumed
...
The main-text comparison and the following fourteen plates include all fifteen
distinct figures generated during the investigation. A PNG and PDF export of
the same plot are one figure;
...
geodesics. Figures~\ref{fig:s3d}--\ref{fig:sshape} concern the saddle; the main-text
Figure~\ref{fig:scompare} compares it with the paraboloid. Figure~\ref{fig:scaling} resolves the axis-scaling
...
under both sampling measures. Over the tested range, relative distance error
appears to approach different positive levels, while the third-to-second singular-value ratio remains near
