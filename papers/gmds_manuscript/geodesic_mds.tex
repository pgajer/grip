...
wireframe includes the full disk boundary. Coordinates are centered, divided
by $r^2$ and aligned; fitted coordinates are additionally divided by their
edge calibration factor. All panels use common limits and equal axis scales
(Appendix~\ref{app:rendering}). At $r=64$ the original disk's horizontal
radius in these units is $1/64$.}\label{fig:spatial}
\end{figure}
...
\subsection{Rendering of Figure~\ref{fig:spatial}}\label{app:rendering}
All coordinates are centered and divided uniformly by $r^2$; fitted
coordinates are also divided by their edge calibration factor and
orthogonally aligned to the original observations. Each row uses common
limits, equal spatial axes and an orthographic view. No coordinate direction
is enlarged independently, so the narrow appearance of the original disk at
$r=64$ reflects its horizontal radius of $1/64$ in these units.

\section*{Data and code availability}
The manuscript's figures and tables are generated from versioned exports of
the two experiments, both run with the \texttt{grip} R package. A source manifest records the exact commit and
