...
graph routes before and after embedding; whether either criterion implies
recovery of the sampled coordinates is rarely assessed. We examine this
question through theory and controlled comparisons of classical scaling,
metric stress MDS, and edge-length refinement toward the graph-geodesic MDS
(GMDS) path criterion on paraboloids and saddles.
...
does not consistently lower coordinate error. At the largest saddle radius,
fixed-scale fits to geodesic graphs reach \numGeodesicSaddlePath\% median path-to-surface
error with \numGeodesicSaddleCoord\% coordinate error, whereas ambient-chord
graphs reverse this pattern (\numAmbientSaddlePath\% and
\numAmbientSaddleCoord\%). Profiling a common target scale in an unnormalized
...
fitted surface becomes planar or linear: Section~\ref{sec:limits} shows
that, under the same loss, the expanding paraboloid approaches a line while
the expanding saddle retains three dimensions, because their input distance
geometries differ.
...
Samples & five, $n=1{,}000$, area-uniform & three, $n=240$, base-disk and surface-area uniform; one nested $n=480$\\\addlinespace
...
Primary diagnostics & retained-path, edge, chord and coordinate errors & path-to-surface, coordinate and component errors, shape ratios, calibration factor\\
\bottomrule
\end{tabular}
\end{table}
...
\section{Results}
Throughout this section, counts and medians describe the declared graph
grids at their achieved optimization budgets. They are not independent
replications, uncertainty estimates for a preferred setting, or certificates
of global optimality; they are also not probabilities for future data.
...
the two graphs where classical initialization ends lower are reported, not
excluded.
...
by $r^2$ and aligned as described in Appendix~\ref{app:rendering}; at
$r=64$ the original disk's horizontal radius in these units is
$1/64$.}\label{fig:spatial}
\end{figure}
...
certify one another. A small retained-path error does not certify coordinate
...
residual. Initialization or regularization can select among configurations
...
because height dominates the coordinate norm. Both measurements also inherit
the graph's approximation to the underlying geometry: a drawing that
faithfully represents a chosen graph still inherits that approximation, and
