...
test_that("build.misf returns nested deterministic levels on a mesh", {
  ...
  misf1 <- build.misf(edges = edges, n = 16L, num.init = 6L, num.nbrs = 8L, seed = 11L)
  misf2 <- build.misf(edges = edges, n = 16L, num.init = 6L, num.nbrs = 8L, seed = 11L)
...
test_that("build.misf handles small graphs with a single MISF level", {
  ...
  misf <- build.misf(edges = edges, n = 5L, num.init = 8L, num.nbrs = 4L, seed = 3L)
...
test_that("build.misf accepts weighted graph input and matches adj-list mode", {
  ...
  built <- grip:::grip.build.adj.from.edges(edges, n = 5L, edge.weights = edge.weights)
  ...
  misf.adj <- build.misf(
    adj.list = built$adj_list,
    weight.list = built$weight_list,
    n = 5L,
    num.init = 3L,
    num.nbrs = 4L,
    seed = 7L
