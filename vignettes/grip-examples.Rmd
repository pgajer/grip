...
**Overview**  
This vignette demonstrates `grip.layout()` on common graph families inspired by
the legacy GRIP examples. We show both 2D and 3D layouts and include small
parameter settings so the examples run quickly.
...
**Helper Functions**  
We define small helpers to generate edge lists for the examples.
...
**2D Examples**  
Circle placement is available only in 2D.
...
**3D Examples**  
The layout output is a numeric matrix with 3 columns. Plotting is optional and
depends on your preferred 3D library.
...
If your data comes as an adjacency list with parallel edge weights, you can
pass it directly. `weight_list` is optional; omit it to treat all weights as 1.
