...
karate.search <- compare.layouts(
  ...
  search = list(
    ...
    preset = c("tree"),
    rounds = c(96L, 128L),
    final.rounds = c(192L, 224L),
    repulsion.factor = c(1.25, 2.0)
  ),
  clusters = karate.club,
  seeds = 1:3,
  sample.size.stress = 1000L,
...
weighted.candidates <- list(
  weighted_default = grip(metric = "edge_length",
    adj.list = hmp.u01.gc.coarse$adj_list,
    weight.list = hmp.u01.gc.coarse$weight_list,
...
because they assess retained-path length fidelity separately from sampled
endpoint-distance stress. These score values are not interchangeable. These GKK/LGKK helpers are public,
