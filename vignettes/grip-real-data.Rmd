...
1. a **combinatorial workflow** for unweighted or topology-first graphs, based
   on `grip.compare.layouts()` and `grip.score.layout()`,
2. a **weighted workflow** for graphs whose edge lengths carry real geometry,
   based on `grip.layout.weighted()`, weighted scoring, and often 3D layouts.
...
- ordinary unweighted or topology-first graphs: start with `grip.layout()`,
  `grip.compare.layouts()`, and `grip.score.layout()`,
- weighted graphs with meaningful edge lengths: start with
  `grip.layout.weighted()`, usually with 3D in the candidate set,
...
karate.cmp <- grip.compare.layouts(
  edges = karate.edges,
  n = karate.n,
  dim = 3,
...
```{r}
karate.search <- grip.compare.layouts(
  edges = karate.edges,
  n = karate.n,
  dim = 3,
  search = list(
...
grip.score.layout(kite.coords, edges = kite.edges, n = kite.n)
```
...
`grip.score.layout()` is especially useful when:
...
- start with `grip.layout.weighted()` rather than `grip.layout()`,
- prefer 3D as the primary layout space,
- and assess geometry with a weighted graph-metric criterion when possible.

A plain `grip.layout()` solve can still be useful here as a topology-first
...
weighted.candidates <- list(
  weighted_default = grip.layout.weighted(
  ...
  weighted_irregular = grip.layout.weighted(
    adj_list = hmp.u01.gc.coarse$adj_list,
    weight_list = hmp.u01.gc.coarse$weight_list,
    n = length(hmp.u01.gc.coarse$adj_list),
    dim = 3,
...
- Use `grip.compare.layouts()` for unweighted or topology-first real graphs.
...
- Use `grip.layout.trace()` or `grip.layout.trace.weighted()` when a promising
